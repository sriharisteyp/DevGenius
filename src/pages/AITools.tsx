...
const MONTHLY_CREDIT_LIMIT = 15; // Monthly credit limit per user

const formatOutput = (text: string): string => {
  // Replace markdown-style code blocks with HTML-style formatting
  let formatted = text.replace(/```(\w+)?\n([\s\S]*?)```/g, (_, lang, code) => {
    return `<pre class="bg-gray-800 p-4 rounded-lg overflow-x-auto"><code class="language-${lang || 'text'}">${code.trim()}</code></pre>`;
  });
...
const AITools = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black text-white">
      ...
      {/* Title Section */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 text-center pt-[100px]">
        ...
        <p className="text-gray-400 text-lg md:text-xl max-w-2xl mx-auto">
          Powerful AI-powered tools to enhance your development workflow and boost productivity
        </p>
      </div>
              ...
              <div className="flex items-center gap-4">
                <div className="text-sm">
                  {credits.used <= 7 ? (
                    ...
                    <span className="text-yellow-400">Credits running low</span>
                  ) : (
                    <span className="text-red-400">Critical: Almost out of credits</span>
                  )}
                </div>
                {credits.used >= 10 && (
                  <Button
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <Card className="bg-card/30 backdrop-blur-sm border-green-500/30">
            ...
            <CardContent>
              <div 
                className="min-h-[200px] p-4 bg-background/50 rounded-lg border border-gray-800 text-white font-mono overflow-auto prose prose-invert max-w-none"
                dangerouslySetInnerHTML={{ 
                  __html: result ? formatOutput(result) : "Generated content will appear here..." 
                }}
              />
            </CardContent>
          </Card>
